    ...
    trapFocus: function(options) {
      ...
      $(document).on(eventName, function(evt) {
        if (options.$container[0] !== evt.target && !options.$container.has(evt.target).length) {
          // Flatheads custom code: commented out below line
          // options.$container.focus();
          // END Flatheads custom code: commented out above line
        }
      });
    },
  ...
  slate.Variants = (function() {
    ...
    Variants.prototype = $.extend({}, Variants.prototype, {
      ...
      _onSelectChange: function() {
        ...
        // Flatheads custom code begin
        this._updateVariantMsg(variant);
        // Flatheads custom code end
      ...
      // Flatheads custom code begin
      _updateVariantMsg: function(variant) {
        this.$container.trigger({
          type: 'variantMsgChange',
          variant: variant
        });
      },
      // Flatheads custom code end
  ...
  theme.Product = (function() {
    ...
    function Product(container) {
      ...
      this.selectors = {
        ...
        // Flatheads custom code begin
        variantMsg: '#customVariantMsg',
        variantMsgWrapper: '#variantMsgWrapper',
        // Flatheads custom code end
    ...
    Product.prototype = $.extend({}, Product.prototype, {
      init: function() {
        ...
        if (!this.inModal) {
          ...
          theme.initFeaturesCarousel(this.$container.find('.features-carousel'));
  
          // Add product id to recently viewed array
          this.addIdToRecentlyViewed();

          this.showFHCustomMessaging();
        }
      },

      showFHCustomMessaging: function() {
        var fhCustomMsgAudSeg = Cookies.get('fhCustomMsgAudSeg');
        if(fhCustomMsgAudSeg == 'FB-FH-28') {
          $('#welcomeDiscountMsg').html('- Get additional <span style="font-weight:600;">15%</span> off. Use code <span style="font-weight:600;">GET15</span>.<br>');
        }
        
        var fhCustomMsgAudSeg2 = Cookies.get('fhCustomMsgAudSeg2');
        if(fhCustomMsgAudSeg2 == 'FB-FH-__21' && Date.now() < 1614018599000) {
          $('#fhCustomAudSegMsg2').html('- Get additional <span style="font-weight:600;">₹999</span> off on size 9. Use code <span style="font-weight:600;">SIZE9</span>.<br>');
        }
        
        
        if(fhCustomMsgAudSeg == 't-shirt-atc' && Date.now() < 1613673000000) {
//           $('#welcomeDiscCode').html('FREETEE');
//           $('#welcomeDiscVal').html('₹1000');
          $('#welcomeDiscountMsg').hide();
          $('#fhCustomAudSegMsg').html('- Get additional <span style="font-weight:600;">FLAT ₹1000</span> off and a <span style="font-weight:600;">FREE Bamboo T-shirt</span>. Use code <span style="font-weight:600;">FREETEE</span>.<br>');
        }
      ...
      initVariants: function() {
        ...
        this.$container
          ...
          // Flatheads custom code begin
          .on('variantMsgChange' + this.settings.namespace, this.updateVariantMsg.bind(this));
          // Flatheads custom code end
        ...
        this.showVariantMsgs(this.variants.currentVariant);
      ...
      updateCartButton: function(evt) {
        ...
        if (variant) {
          if (variant.available) {
            ...
            if (variant.sku.includes("TRS")) {
				$(this.selectors.addToCartText).html("PRE-ORDER NOW");
            }
            else {
      ...
      // Flatheads custom code begin
      updateVariantMsg: function(evt) {
        var variant = evt.variant;
        this.showVariantMsgs(variant);
      },
      // Flatheads custom code end
      ...
      showVariantMsgs: function(variant) {
//         if (variant && variant.sku.includes('ELL')) {
//           if (variant.sku.includes('ASH') || variant.sku.includes('GRA')) {
//               if (variant.title.includes('12')) {
//                 $(this.selectors.variantMsgWrapper).removeClass('hide');
//                 $(this.selectors.variantMsg).html("This size will be delivered in 7-10 days.");
//                 } else {
//                   $(this.selectors.variantMsgWrapper).addClass('hide');
//                   $(this.selectors.variantMsg).html("");
//                 }
//           }
//           else if (variant.sku.includes('BUR')) {
//               if (variant.title.includes('12')) {
//                 $(this.selectors.variantMsgWrapper).removeClass('hide');
//                 $(this.selectors.variantMsg).html("This size will be delivered in 7-10 days.");
//                 } else {
...
//                 }
//           }
//           else if (variant.sku.includes('COB')) {
//               if (variant.title.includes('6')) {
//                 $(this.selectors.variantMsgWrapper).removeClass('hide');
//                 $(this.selectors.variantMsg).html("This size will be delivered in 7-10 days.");
//                 } else {
//                   $(this.selectors.variantMsgWrapper).addClass('hide');
  ...
  theme.Collection = (function() {
    ...
    Collection.prototype = $.extend({}, Collection.prototype, {
      ...
      showFHCustomMessaging: function() {
        ...
        if(fhCustomMsgAudSeg == 'FB-FH-28') {
          $('#fhCustomDiscMsg').html('<p style="font-size: 18px; background-color: #ffdcdc; padding: 5px; text-align: center;">Get additional 15% off. Use code <span style="font-weight:600;">GET15</span></p>');
        }
        
        if(fhCustomMsgAudSeg2 == 'FB-FH-__21' && Date.now() < 1614018599000) {
          $('#fhCustomDiscMsg').html('<p style="font-size: 18px; background-color: #ffdcdc; padding: 5px; text-align: center;">Get additional <span style="font-weight:600;">₹999</span> off on size 9. Use code <span style="font-weight:600;">SIZE9</span></p>');
        }
...
//         }
  ...
  /* Flatheads custom code features carousel begin */

  theme.initFeaturesCarousel = function(elem, settings) {
  ...
  theme.initInitialCustomPageScripts = function() {
    if(window.location.search.includes('utm_campaign')) {
      var urlParams = new URLSearchParams(window.location.search);
      var utmCampaign = urlParams.get('utm_campaign');
      
      if(urlParams.get('utm_source') && urlParams.get('utm_source').includes('Deals102')) {
        Cookies.set('affiliateSubID', urlParams.get('utm_campaign'), { path: '/', expires: 1 });
      }

      if(utmCampaign.includes('FB-FH-28')) {
        var fhCustomMsgAudSeg = Cookies.get('fhCustomMsgAudSeg');
        if(!(fhCustomMsgAudSeg && fhCustomMsgAudSeg == 'FB-FH-28')) {
          Cookies.set('fhCustomMsgAudSeg', 'FB-FH-28', { path: '/', expires: 10000 });
        }
      }
      
      if(utmCampaign.includes('FB-FH-23') || utmCampaign.includes('FB-FH-11') || utmCampaign.includes('FB-FH-15') || utmCampaign.includes('FB-FH-27')) {
        var fhCustomMsgAudSeg2 = Cookies.get('fhCustomMsgAudSeg2');
        if(!(fhCustomMsgAudSeg2 && fhCustomMsgAudSeg2 == 'FB-FH-__21')) {
          Cookies.set('fhCustomMsgAudSeg2', 'FB-FH-__21', { path: '/', expires: 10000 });
        }
      }
      
      if(utmCampaign.includes('t-shirt-atc')) {
        var fhCustomMsgAudSeg = Cookies.get('fhCustomMsgAudSeg');
        if(!(fhCustomMsgAudSeg && fhCustomMsgAudSeg == 't-shirt-atc')) {
          Cookies.set('fhCustomMsgAudSeg', 't-shirt-atc', { path: '/', expires: 10000 });
        }
      }
  ...
  theme.initCustomPageScripts = function() {
    if(window.location.pathname.includes('features-reviews-collection') || window.location.pathname.includes('pages/thank-you')) {
      theme.initFeaturesCarousel($('.features-carousel'));
      theme.initFeaturesCarousel($('.ugc-carousel'), {
          infinite: true,
          dots: true,
          arrows: false,
          autoplay: true,
  ...
  theme.init = function() {
    theme.initInitialCustomPageScripts();
    theme.setGlobals();
    theme.pageTransitions();
